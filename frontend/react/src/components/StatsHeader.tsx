...
const StatsHeader: React.FC<StatsHeaderProps> = ({ username, from }) => {
	...
	return (
		<div>
			<h1 className="text-6xl text-center text-teal-800 dark:text-teal-300 m-5 p-5">{t('stats.title', { username })}</h1>
			<p className="text-center">
				<button className="block mx-auto px-20 text-white bg-teal-700 hover:bg-teal-800 focus:ring-4 
								  ...
								  dark:focus:ring-teal-800" onClick={() => navigate(from || '/')}>{t('statsHeader.back')}</button>
			</p>
		</div>
	);
};
